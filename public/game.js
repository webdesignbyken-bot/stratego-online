...
(() => {
  ...
  let mode = null; // local | online | ai
  let nickname = '';
  let myColor = null;
  let socket = null;
  let game = null; // server state snapshot for online, full state for local/ai
  let selectedTrayRank = null;
  let selectedSquare = null;

  // Local / AI full state
  let localGame = null;
  // AI memory of revealed opponent ranks: key "r,c" -> rank
  let aiKnown = {};
  ...
  function startAi() {
    ...
    localGame = {
      ...
      remaining: { red: createRemaining(), blue: createRemaining() },
      captured: { red: [], blue: [] },
      names: { red: nickname || 'You', blue: 'Computer' }
    };
    game = snapshotLocal();
    showGame();
    buildBoard();
    ...
    updateUI();
    updatePlacementButtons();
    setMessage('Place your pieces (bottom 4 rows), then click I\'m Ready. Computer will set up automatically.');
    addLog('vs Computer – you are Red.');
  }

  function aiPlaceSetup() {
    // Strategic blue placement on rows 0–3
    const board = localGame.board;
    const rem = createRemaining();
    const empty = [];
    for (let r = 0; r <= 3; r++) {
      for (let c = 0; c < 10; c++) {
        if (!LAKES.has(`${r},${c}`)) empty.push({ r, c });
      }
    }
    const take = (rank, r, c) => {
      if (rem[rank] <= 0) return false;
      if (r < 0 || r > 3 || c < 0 || c > 9 || LAKES.has(`${r},${c}`) || board[r][c]) return false;
      board[r][c] = { rank, color: 'blue', revealed: false };
      rem[rank]--;
      return true;
    };
    // Flag in back row, not edge-center only
    const flagSpots = [
      { r: 0, c: 1 }, { r: 0, c: 8 }, { r: 0, c: 0 }, { r: 0, c: 9 },
      { r: 0, c: 2 }, { r: 0, c: 7 }, { r: 1, c: 1 }, { r: 1, c: 8 }
    ];
    let flagPos = null;
    for (const p of flagSpots) {
      if (take('F', p.r, p.c)) { flagPos = p; break; }
    }
    if (!flagPos) {
    ...
    // Bombs around flag + some front
    const bombNear = [];
    if (flagPos) {
      for (const [dr, dc] of [[0,1],[0,-1],[1,0],[1,1],[1,-1],[0,2],[0,-2]]) {
        bombNear.push({ r: flagPos.r + dr, c: flagPos.c + dc });
      }
    }
    for (const p of bombNear) take('B', p.r, p.c);
    // Front-line bombs (row 3)
    for (const c of [2, 3, 6, 7, 4, 5, 1, 8]) take('B', 3, c);
    // Strong pieces toward middle/front
    for (const rank of ['1', '2', '3', '3', '4', '4', '4']) {
      const prefs = [
        { r: 2, c: 4 }, { r: 2, c: 5 }, { r: 3, c: 4 }, { r: 3, c: 5 },
        { r: 2, c: 3 }, { r: 2, c: 6 }, { r: 1, c: 4 }, { r: 1, c: 5 },
        { r: 3, c: 1 }, { r: 3, c: 8 }, { r: 2, c: 2 }, { r: 2, c: 7 }
      ];
      let placed = false;
      for (const p of prefs) {
        if (take(rank, p.r, p.c)) { placed = true; break; }
      }
      if (!placed) {
        for (const p of empty) {
          if (take(rank, p.r, p.c)) break;
        }
      }
    }
    // Spy near back / near strong pieces
    take('S', 1, flagPos ? Math.min(9, flagPos.c + 1) : 4) ||
      take('S', 1, 5) || take('S', 0, 4);
    // Miners mid
    for (let i = 0; i < 5; i++) {
      const prefs = [{ r: 2, c: 1 }, { r: 2, c: 8 }, { r: 2, c: 0 }, { r: 2, c: 9 }, { r: 1, c: 3 }, { r: 1, c: 6 }, { r: 3, c: 0 }, { r: 3, c: 9 }];
      let placed = false;
      for (const p of prefs) {
        if (take('8', p.r, p.c)) { placed = true; break; }
      }
      if (!placed) {
        for (const p of empty) {
          if (take('8', p.r, p.c)) break;
        }
      }
    }
    ...
    for (const rank of ORDER) {
      for (let i = 0; i < rem[rank]; i++) left.push(rank);
    }
    const free = [];
    for (let r = 0; r <= 3; r++) {
      for (let c = 0; c < 10; c++) {
        if (!board[r][c] && !LAKES.has(`${r},${c}`)) free.push({ r, c });
      }
    }
    ...
    for (const rank of left) {
      ...
      const p = free[fi++];
      board[p.r][p.c] = { rank, color: 'blue', revealed: false };
      rem[rank]--;
    }
    localGame.remaining.blue = rem;
  }

  function rankStrength(rank) {
    if (rank === 'S') return 0.5;
    if (rank === 'B' || rank === 'F') return -1;
    const n = parseInt(rank, 10);
    return Number.isFinite(n) ? (11 - n) : 0;
  }
  ...
  function aiChooseMove() {
    ...
    for (let r = 0; r < 10; r++) {
      for (let c = 0; c < 10; c++) {
        ...
        if (!RANKS[piece.rank] || !RANKS[piece.rank].movable) continue;
        const moves = getValidMoves(board, r, c, piece);
        for (const m of moves) {
          candidates.push({ fr: r, fc: c, tr: m.r, tc: m.c, piece });
        }
      }
    }
    ...
    const scoreMove = (mv) => {
      const { fr, fc, tr, tc, piece } = mv;
      let score = Math.random() * 3; // slight noise
      const target = board[tr][tc];
      const myStr = rankStrength(piece.rank);

      // Prefer advancing toward red (higher row numbers for blue)
      score += (tr - fr) * 2.5;
      // Slight center preference
      score += (5 - Math.abs(tc - 4.5)) * 0.3;

      if (!target) {
        // Empty square – exploration / advance
        if (piece.rank === '9') score += 1.5; // scouts explore
        if (tr >= 6) score += 4; // deep into red territory
        if (tr >= 4) score += 1.5;
        return score;
      }
      ...
      // Attack
      const key = `${tr},${tc}`;
      const known = aiKnown[key];
      if (known) {
        if (known === 'F') return score + 1000;
        if (known === 'B') {
          return piece.rank === '8' ? score + 80 : score - 50;
        }
        if (piece.rank === 'S' && known === '1') return score + 90;
        const theirStr = rankStrength(known);
        if (myStr > theirStr) return score + 40 + (myStr - theirStr) * 5;
        if (myStr === theirStr) return score - 5;
        return score - 40 - (theirStr - myStr) * 5;
      }

      // Unknown enemy piece
      if (piece.rank === '8') score += 8; // miner probes
      if (piece.rank === '9') score += 5; // scout probes
      if (myStr >= 8) score += 6; // strong pieces can risk
      if (myStr <= 3) score -= 8; // weak pieces avoid unknown
      if (piece.rank === 'S') score -= 15; // don't waste spy on unknown
      if (piece.rank === '1') score -= 10; // protect marshal
      // Prefer attacking pieces still in red home / mid
      if (tr >= 5) score += 4;
      return score + 10;
    };

    candidates.sort((a, b) => scoreMove(b) - scoreMove(a));
  ...
  function aiTakeTurn() {
    if (mode !== 'ai' || !localGame || localGame.phase !== 'playing') return;
    if (localGame.turn !== 'blue') return;
    ...
    setTimeout(() => {
      aiThinking = false;
      if (mode !== 'ai' || !localGame || localGame.phase !== 'playing' || localGame.turn !== 'blue') return;
      ...
      if (!mv) {
        ...
        game = snapshotLocal();
        renderBoard();
        showWinner('red', 'Computer has no movable pieces.');
        return;
      }
      attemptMove(mv.fr, mv.fc, mv.tr, mv.tc);
    }, 500 + Math.random() * 500);
  }
  ...
  function onReady() {
    ...
    if (mode === 'ai') {
      localGame.ready.red = true;
      setMessage('Computer is setting up…');
      addLog('You are ready. Computer placing pieces…');
      aiPlaceSetup();
      localGame.ready.blue = true;
      ...
      addLog('Game started. You (Red) move first.');
      return;
    }
    // Local sequential
  ...
  function handlePlayClick(r, c) {
    if (mode === 'ai' && (aiThinking || (localGame && localGame.turn === 'blue'))) {
      setMessage('Wait for the computer…');
      return;
    }
    const turn = (mode === 'local' || mode === 'ai')
      ? (localGame ? localGame.turn : game.turn)
      : game.turn;
    const activeColor = (mode === 'local')
      ? turn
      : myColor;
    if (turn !== activeColor && mode !== 'local') {
      setMessage('Not your turn.');
      return;
    }

    const board = (mode === 'local' || mode === 'ai') ? localGame.board : game.board;
    const piece = board[r][c];
  ...
  function attemptMove(fr, fc, tr, tc) {
    ...
    // Hide ranks revealed on a previous turn; combat winners this turn stay visible until next play
    for (let r = 0; r < 10; r++) {
      for (let c = 0; c < 10; c++) {
    ...
    const side = piece.color === 'red' ? 'Red' : 'Blue';
    if (!defender) {
      board[tr][tc] = piece;
      board[fr][fc] = null;
      addLog(`${side} moved.`);
      ...
      // AI remembers revealed ranks
      if (mode === 'ai') {
        if (piece.color === 'red') aiKnown[`${fr},${fc}`] = piece.rank;
        if (defender.color === 'red') aiKnown[`${tr},${tc}`] = defender.rank;
      }
      ...
      if (outcome === 'attacker') {
        ...
        addLog(`${side} ${RANKS[piece.rank].name} captures ${RANKS[defender.rank].name}!`);
        if (mode === 'ai' && piece.color === 'blue') {
          // Piece moved; update known key for survivor location
          delete aiKnown[`${fr},${fc}`];
        }
        if (defender.rank === 'F') {
          localGame.phase = 'ended';
          game = snapshotLocal();
        ...
        addLog(`${defender.color === 'red' ? 'Red' : 'Blue'} ${RANKS[defender.rank].name} defends and captures ${RANKS[piece.rank].name}!`);
      } else {
        board[fr][fc] = null;
        board[tr][tc] = null;
        localGame.captured[piece.color].push(defender.rank);
    ...
    // No moves left?
    if (!hasMovablePieces(localGame.board, localGame.turn)) {
      localGame.phase = 'ended';
      const winner = localGame.turn === 'red' ? 'blue' : 'red';
      game = snapshotLocal();
      renderBoard();
      updateUI();
      updateCaptures();
      showWinner(winner, 'Opponent has no movable pieces.');
      return;
    }
  ...
  function hasMovablePieces(board, color) {
    for (let r = 0; r < 10; r++) {
      for (let c = 0; c < 10; c++) {
        const p = board[r][c];
        if (p && p.color === color && RANKS[p.rank] && RANKS[p.rank].movable) {
          if (getValidMoves(board, r, c, p).length) return true;
        }
      }
    }
    return false;
  ...
  function resign() {
    ...
    if (localGame) {
      const winner = mode === 'ai' ? 'blue' : (myColor === 'red' ? 'blue' : 'red');
      localGame.phase = 'ended';
  ...
  function updateScoreboard() {
    ...
    if (mode === 'online' && game && game.names) {
      ...
      el.textContent = `You ${sessionScores.red} – ${sessionScores.blue} Computer`;
    } else {
  ...
  function showWinner(winner, reason) {
    ...
    if (mode === 'local') {
      ...
      text = winner === 'red' ? '🎉 You Win!' : '🤖 Computer Wins';
    } else {
      text = winner === myColor ? '🎉 You Win!' : 'Defeat';
    }
    ...
    if ($('winner-score')) {
      if (mode === 'online' && game && game.names) {
        ...
        $('winner-score').textContent =
          `Score: You ${sessionScores.red} – ${sessionScores.blue} Computer`;
      } else {
        $('winner-score').textContent =
  ...
  function playAgain() {
    if (mode === 'local' || mode === 'ai') {
      ...
      localGame = {
        ...
        captured: { red: [], blue: [] },
        names: wasAi
          ? { red: nickname || 'You', blue: 'Computer' }
          : { red: 'Red', blue: 'Blue' }
      };
      game = snapshotLocal();
      selectedTrayRank = null;
      ...
      updateScoreboard();
      if (wasAi) {
        setMessage('Place your pieces, then I\'m Ready. Computer sets up automatically.');
        addLog('New game vs Computer. Score carries over.');
      } else {
        setMessage('Red: place your pieces (bottom 4 rows), then Ready.');
  ...
  function renderBoard() {
    ...
    for (let r = 0; r < 10; r++) {
      for (let c = 0; c < 10; c++) {
        ...
        // Whose ranks are visible
        let viewerColor = myColor;
        if (mode === 'local') {
          viewerColor = game.phase === 'placement' ? myColor : (localGame ? localGame.turn : game.turn);
        } else if (mode === 'ai') {
          viewerColor = 'red'; // always show only the human's pieces + revealed
        }
  ...
  function updateUI() {
    ...
    if (mode === 'local') {
      $('player-label').textContent = `Local – ${(localGame ? localGame.turn : myColor || '').toUpperCase()}`;
    } else if (mode === 'ai') {
      $('player-label').textContent = `You (Red) vs Computer`;
    } else {
      $('player-label').textContent = `You are ${(myColor || '').toUpperCase()} (${nickname})${sideNote}`;
    }
    if (game.phase === 'placement') {
      $('turn-indicator').textContent = mode === 'ai' ? 'Place your pieces' : 'Placement phase';
      ...
      if (mode === 'local') {
        ...
        const yours = game.turn === 'red';
        $('turn-indicator').textContent = yours ? 'Your turn' : "Computer's turn";
      } else {
