...
(() => {
  ...
  function init() {
    ...
    $('btn-play-again').onclick = backToLobby;
    $('btn-sync').onclick = () => {
      if (mode === 'online' && socket) socket.emit('request-state');
      else setMessage('Sync only needed for online games.');
    };
    $('btn-random').onclick = randomPlaceAll;
    $('join-code').addEventListener('keydown', e => { if (e.key === 'Enter') joinRoom(); });

    console.log('[Stratego] v2.0 Socket.io client loaded – ranks 1=Marshal … 9=Scout');
  }
  ...
  function startHost() {
    ...
    $('host-status').textContent = 'Status: Connecting…';
    $('room-code').textContent = '…';

    const s = connectSocket();
    const doCreate = () => {
      s.emit('create-room', { name: nickname }, (res) => {
        if (!res || !res.ok) {
  ...
  function showWinner(winner, reason) {
    const text = winner === myColor ? '🎉 You Win!' : 'Defeat';
    $('winner-text').textContent = `${text} – ${reason || ''}`;
  ...
  function renderBoard() {
    ...
    for (let r = 0; r < 10; r++) {
      for (let c = 0; c < 10; c++) {
        ...
        const showRank = mode === 'local' || isMine || cell.revealed || game.phase === 'ended';
  ...
  function backToLobby() {
    ...
    $('host-panel').classList.add('hidden');
    $('join-panel').classList.add('hidden');
    $('room-code').textContent = 'Creating…';
