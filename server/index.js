...
function publicRoomState(room, viewerColor) {
  return {
    ...
    rematchVotes: room.rematchVotes || { red: false, blue: false },
    playersConnected: {
...
function endGame(room, winner, reason) {
  ...
  if (winner === 'red' || winner === 'blue') {
    room.scores = room.scores || { red: 0, blue: 0 };
    room.scores[winner]++;
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('move', ({ fr, fc, tr, tc }, cb) => {
    ...
    // Hide ranks revealed on a previous turn (unless keepRevealed is on).
    // Combat winners from THIS move stay visible until the next play.
    ...
    if (!defender) {
      ...
      logMsg = `${side} moved.`;
      io.to(room.code).emit('sfx', { type: 'move' });
    } else {
      piece.revealed = true;
      defender.revealed = true;
      const outcome = combat(piece, defender);
      io.to(room.code).emit('sfx', { type: defender.rank === 'B' ? 'bomb' : 'combat' });

      if (outcome === 'attacker') {
        room.board[tr][tc] = piece;
        room.board[fr][fc] = null;
        // Winner stays revealed until the next play
        piece.revealed = true;
        room.captured[color].push(defender.rank);
        logMsg = `${side} ${RANKS[piece.rank].name} captures ${RANKS[defender.rank].name}!`;
        if (defender.rank === 'F') {
          endGame(room, color, 'Flag captured!');
          broadcastState(room);
          io.to(room.code).emit('log', logMsg);
          io.to(room.code).emit('game-over', { winner: color, reason: room.reason, scores: room.scores });
          if (typeof cb === 'function') cb({ ok: true });
          return;
        }
    ...
    if (!hasMovable(room.board, room.turn)) {
      endGame(room, color, 'Opponent has no movable pieces.');
      broadcastState(room);
      io.to(room.code).emit('log', logMsg);
      io.to(room.code).emit('game-over', { winner: color, reason: room.reason, scores: room.scores });
      if (typeof cb === 'function') cb({ ok: true });
  ...
  socket.on('resign', () => {
    ...
    endGame(room, winner, `${room.names[meta.color]} resigned.`);
    broadcastState(room);
    io.to(room.code).emit('game-over', { winner, reason: room.reason, scores: room.scores });
  ...
  socket.on('request-rematch', () => {
    ...
    room.rematchVotes = room.rematchVotes || { red: false, blue: false };
    room.rematchVotes[meta.color] = true;

    io.to(room.code).emit('rematch-status', {
      votes: room.rematchVotes,
      names: room.names
    });
    io.to(room.code).emit('log', `${room.names[meta.color]} wants a rematch.`);

    if (room.rematchVotes.red && room.rematchVotes.blue) {
      // Reset board for a new game; keep players, names, scores, keepRevealed
      room.board = emptyBoard();
      room.turn = 'red';
      room.phase = 'placement';
      room.ready = { red: false, blue: false };
      room.remaining = { red: createRemaining(), blue: createRemaining() };
      room.captured = { red: [], blue: [] };
      room.winner = null;
      room.reason = null;
      room.rematchVotes = { red: false, blue: false };
      io.to(room.code).emit('log', 'Rematch accepted! Place your pieces.');
      broadcastState(room);
    }
  });
